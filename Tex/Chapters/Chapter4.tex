...
The \term{GameScript.cs}\footnote{\url{https://github.com/cstenkamp/BA-rAIce/blob/master/Assets/Scripts/GameScript.cs}}\byLeon is only relevant for changing the game mode. Its instanciation during runtime is referred to as \inlinecode{Game}. In its \inlinecode{Start()}-method, it prepares the cameras for the minimap according to the options in \inlinecode{Consts}. After the game mode is changed during runtime (following a press of the \keystroke{Esc}-button), the function \inlinecode{SwitchMode(newMode)} is called. This function disconnects any connected Sockets, and changes a globally known variable \inlinecode{mode} according to the user's selection. \inlinecode{mode} is a string-array that contains one or more elements of the following group: \inlinecode{driving}, \inlinecode{keyboarddriving}, \inlinecode{train\_AI}, \inlinecode{drive\_AI} and \inlinecode{menu}. As many methods in various classes are supposed to behave differently depending on the current game mode, they trigger certain behavioury only if \inlinecode{Game}'s \inlinecode{mode} contains one of those strings\footnote{As mentioned earlier, the apparent modes to the User are \term{Drive}, \term{Drive AI} and \term{train AI supervisedly}, however in the actual implementation the mode is differentiated more fine-grained: all three of those use the \inlinecode{drive} additionally to the respective obvious element. The additional element \inlinecode{keyboarddriving} is active when the driving commands are coming via the keyboard, which includes \term{train AI supervisedly}. This was done, such that if an AI-agent shall also function to generate supervised data, one can simply add the respective mode in the \term{Gamescript.cs}-file.}. After setting the respective mode, \inlinecode{SwitchMode} activates the required cameras for the mode, updates the UIs Display indicating the mode and calls functions to initialize the respective mode (\inlinecode{AiInt.StartedAIMode()} or \inlinecode{Rec-StartedSV\_SaveMode()}), if applicable\footnote{Note that to check if the \term{Drive AI} mode is active, it is often not checked if \inlinecode{mode} contains \inlinecode{drive_AI}, but if the variable \inlinecode{AiInt.AIMode} is true -- this variable is not true until everything for the AiMode is intitialized.}. When the game is started, \inlinecode{SwitchMode()} is called to switch to the \inlinecode{menu}-mode.

The job of the  \term{UIScript.cs}\footnote{\url{https://github.com/cstenkamp/BA-rAIce/blob/master/Assets/Scripts/UIScript.cs}}\byLeon is to update the game's user interface. This UI is overlayed over the current scene, such that its components can be seen simultaneously to the image of an active camera. In its \inlinecode{MenuOverLayHandling()}-Method, it specifies the view of the game's menu-mode (as can be seen in figure~\ref{fig:overviewshot} in appendix~\ref{AppendixB}), as well as the key bindings to activate the required mode. In the method \inlinecode{DrivingOverlayHandling()}, it sets visibility, content, color or position of numerous UI components (defined in Unitys Object Hierachy), that are seen in non-menu-modes. Both \inlinecode{DrivingOverlayHandling()} and \inlinecode{MenuOverLayHandling()} are colled every \inlinecode{Update()}, such that the view elements are always contemporary. Further, the \term{UIScript.cs} specifies the \inlinecode{void onGUI()}-function, which Unity calls every time it re-renders the GUI. This function changes the view if the QuickPause-mode is activated (more on that later).


\subsubsection{User Interface}

The User Interface of the game while driving can be seen in the figures~\ref{fig:humandriveshot} and~\ref{fig:aidriveshot} in appendix~\ref{AppendixB}, which are screenshots for the \inlinecode{keyboarddriving} and \inlinecode{drive_AI} mode, respectively. The latter of those screenshots is annotated with labelled boxes around each UI component, with page~\pageref{fig:aidriveshot} explaining each component a bit further\footnote{Note that the entire UI, besides the content behind labels A, F, H and I, was already implemented like this by \leon.}.

\subsubsection{Controls}

In the \term{keyboarddriving}-mode, the game is steered with the arrow keys \keystroke{$\leftarrow$} and \mbox{\keystroke{$\rightarrow$}.} The throttle is triggered via the \keystroke{A}-key, whereas the brake is called via \keystroke{Y}. The \keystroke{R} - key flips the reverse gear. Note that as long as the \inlinecode{pedalType} in \term{CarController.cs}\footnote{\url{https://github.com/cstenkamp/BA-rAIce/blob/master/Assets/Scripts/CarController.cs}}\byLeon set to \inlinecode{digital}, throttle and brake are binary when controlled via keyboard. In the \inlinecode{drive\_AI} mode, the car is usually controlled by the agent, it is however possible to re-gain control over it via pressing the \keystroke{H}-key. This is useful for example if one wants to check if rewards or Q-values are realistic. If human interference of the \inlinecode{drive\_AI} mode is active, it is possible to simulate speeds to the agent (meaning that not the actual speed of the car, but a specified value is sent via Sockets). This can be done with the Number keys, where the pretended speed is evenly spread between $0 kph$ (\keystroke{0}) and $250 kph$ (\keystroke{9}). The \keystroke{P} key is reserved to simulate a full throttle value. To hand control back to the agent, \keystroke{H} must be pressed again. 

During any mode containing \inlinecode{driving}, \keystroke{Q} can be pressed to activate the \term{QuickPause}-mode. If Quickpause is activated, Unity's \term{time scale} is set to zero, which means that calls of \inlinecode{FixedUpdate()} and its subsequent calls are stopped. This freezes Unity's entire physics, which allows to spectate the current screen. Another very useful effect of \term{QuickPause} is, that side threads are not stopped, giving them time to catch up with the progress of the rest of the game (more on that later). QuickPause is ended with another hit of \keystroke{Q}. 

In the \inlinecode{drive\_AI}-mode, a user can manually disconnect or attempt a connection-trial with an agent. The keys to do that are \keystroke{D} and \keystroke{C}, respectively.

\subsection{The car}

How the car moves is specified in the MonoBehaviour specified in \term{CarController.cs}. In modes containing \inlinecode{driving}, \inlinecode{FixedUpdate()}-step friction is adjusted according to the current surface the wheels are on and it is checked if the car moved outside the street's surface, its velocity as well as some other values are calculated. Finally, the torques for acceleration and braking are applied and the front wheels are turned according to the steering-value. The amount of those torques and angles depend on a value $\in [0,1]$ for throttle and brake and $\in [-1,1]$ for the steering -- if the mode contains \inlinecode{keyboarddriving} or \inlinecode{HumanTakingControl} is active, those values depend on the User's keyboard input, if \inlinecode{AiInt.AIMode} is enabled, the values are forwarded from the \inlinecode{AiInt} (values \inlinecode{nn\_steer}, \inlinecode{nn\_throttle} and \inlinecode{nn_throttle}). The \inlinecode{FixedUpdate()}-method of the car also sets all car's forces to zero if 


dann die anderen functions yaay
